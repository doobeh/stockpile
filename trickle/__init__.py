...
def reader_old(location, dt):
    product_sales = list()
    totals = 0
    files = [os.path.join(location, x) for x in os.listdir(location) if dt.strftime('TRPER%Y%m%d') in x]
    for xml_file in files:
        file_sales, ticket_total = process_file_old(xml_file, dt)
        product_sales += file_sales
        totals += ticket_total
    print totals / 100.0
    return product_sales
...
def process_file_new(xml_file, dt):
...
def process_file_old(xml_file, dt):
    """
    TicketTotals don't exist in the old trickle format, so we can't skip voided
    transactions.
    """
    product_sales = list()
